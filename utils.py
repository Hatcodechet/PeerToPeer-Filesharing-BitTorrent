...
def set_socket(port):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # AF_INET IPv4, SOCK_DGRAM:  UDP (không kết nối).
...
def parse_command(command):
    """
    Parses the user command and extracts mode and filename.
    Expected format: torrent -mode <MODE> <FILENAME>
    """
    parts = command.split()
    if len(parts) < 3 or parts[0] != "torrent" or parts[1] != "-mode":
        print("Invalid command. Expected format: torrent -mode <MODE> <FILENAME>")
        return None, None
    mode = parts[2]
