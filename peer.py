...
class Peer:
    ...
    def send_segment(self, sock, data, addr):
        try:
            ...
            if not isinstance(addr, tuple) or len(addr) != 2:
                raise ValueError(f"[ERROR] Invalid address format: {addr}")
            print(f"[DEBUG] Sending data to {addr}")  # Log địa chỉ IP và cổng đích

            # Gửi dữ liệu
            sock.sendto(data, addr)
        except Exception as e:
            print(f"[ERROR] Failed to send data to {addr}: {e}")
            raise
    ...
    def listen(self):
        while True:
            try:
                ...
                print(f"[DEBUG] Received data from {addr}")  # Log địa chỉ Peer gửi
                msg = Message.decode(data)
                self.handle_requests(msg, addr)
            except Exception as e:
                print(f"[ERROR] Error in listening for connections: {e}")


    def set_send_mode(self, filename):
        if filename not in self.files: # check peer keep file or not???
            log_content = f"You don't own the file {filename}"
            log(peer_id = self.peer_id, content = log_content)
            return
        #annouce to tracker, peer is keeping file
    ...
    def split_file_owners(self, file_owners, filename):
        ...
        to_be_used_owners = owners[:config.constants.MAX_CONCURRENT_REQUESTS] #max num of owners
        ...
        for idx, owner in enumerate(to_be_used_owners):
            ...
            thread.setDaemon(True) # thread will be automatically canceled if program ends
            thread.start()
            neighboring_peers_threads.append(thread)
        for thread in neighboring_peers_threads:
            thread.join() #Thread run song song -> tang toc qua trinh download
            #main prog wait all thread downloading before continuing to handle with chunkPieces
